...
describe.skipIf(!fixtureAvailable)(
  "ConvenienceRoundTracker — integration on de_nuke.dem via DemoParser.parse()",
  () => {
    it("produces a reasonable RoundSummary array with sensible field values", async () => {
      ...
      const roundsWithPlants = result.rounds.filter((r) => r.bombEvents.plants.length > 0);
      ...
      console.log(`Total kills: ${result.kills.length}, round-kills total: ${roundKillsTotal}`);
