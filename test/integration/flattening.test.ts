...
import type { ServerClass } from "../../src/datatables/ServerClass.js";
import type { ServerClassRegistry } from "../../src/datatables/ServerClassRegistry.js";
...
describe.skipIf(!fixtureAvailable)(
  "SendTable flattening — golden cross-check against de_nuke.dem",
  () => {
    // Parse once, share across the suite. All fixture I/O lives in beforeAll (not
    // the describe body) so a missing fixture skips cleanly instead of crashing
    // at collection time. The shared classes are resolved here too.
    let classes: ServerClassRegistry;
    let player: ServerClass;
    let team: ServerClass;
    let wep: ServerClass;
    let gr: ServerClass;
    beforeAll(() => {
      const parser = DemoParser.fromFile(FIXTURE_PATH);
      parser.parseAll();
      classes = parser.serverClasses!;
      player = classes.byName("CCSPlayer")!;
      team = classes.byName("CCSTeam")!;
      wep = classes.byName("CWeaponCSBase")!;
      gr = classes.byName("CCSGameRulesProxy")!;
    });
