...
describe.skipIf(!fixtureAvailable)(
  "Miscellaneous game-state events (Tier-1) — integration on de_nuke.dem",
  () => {
    it("emits typed misc match-state events", () => {
      ...
      parser.on("announce_phase_end", (e: AnnouncePhaseEndEvent) => announcePhaseEnd.push(e));
      parser.on("cs_win_panel_match", (e: CsWinPanelMatchEvent) => csWinPanelMatch.push(e));
      parser.on("cs_win_panel_round", (e: CsWinPanelRoundEvent) => csWinPanelRound.push(e));
      parser.on("match_end_conditions", (e: MatchEndConditionsEvent) => matchEndConditions.push(e));
